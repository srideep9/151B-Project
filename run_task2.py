...
def resolve_lora_adapter(adapter: Optional[str]) -> Optional[str]:
    if adapter is None:
        return None
    adapter = adapter.strip()
    if not adapter:
        return None
    if adapter in LORA_ADAPTERS:
        return LORA_ADAPTERS[adapter]
    return adapter


def download_lora_adapter(adapter_id: Optional[str]) -> Optional[str]:
    if not adapter_id:
        return None
    ...
    return snapshot_download(repo_id=adapter_id, token=token)
...
def run_inference(
    ...
    num_outputs: int = 1,
    output_path: str = "outputs/run_task2_predictions.jsonl",
    temperature: float = 0,
    top_p: float = 0.95,
    max_tokens: int = 8192,
    max_num_seqs: int = 64,
    ...
    """Run the full inference/evaluation pipeline with an optional LoRA adapter.

    `adapter` can be one of: none, public, private, public_private, or a HF/local
    adapter path. This is the single entry point expected by final submission.
    """
    class Args:
        pass

    args = Args()
    args.eval = evaluation
    ...
    GPU_ID      = "0"
    DATA_PATH   = data_path or ("data/public.jsonl" if evaluation else "data/private.jsonl")
    OUTPUT_PATH = args.output_path
    MAX_TOKENS  = args.max_tokens

    os.environ["CUDA_VISIBLE_DEVICES"] = GPU_ID

    adapter_id = resolve_lora_adapter(args.adapter)
    adapter_path = download_lora_adapter(adapter_id)
    lora_request = None
    if adapter_path:
        from vllm.lora.request import LoRARequest

        lora_request = LoRARequest("child_lora_adapter", 1, adapter_path)
        print(f"Using LoRA adapter: {adapter_id} -> {adapter_path}")
    ...
    if args.use_wandb:
        wandb.init(
            ...
            name="eval-01vote-lora",
            tags=["voting", "public-data", "lora"],
            config={
                "model_id": MODEL_ID,
...
\boxed{A}"""}
    ...
    if evaluation:
        prediction_table = (
            wandb.Table(columns=["ID", "Type", "Ground Truth", "Model Response", "Correct"])
            if args.use_wandb
            else None
        )
        ...
        prediction_table = (
            wandb.Table(columns=["ID", "Type", "Model Response"])
            if args.use_wandb
            else None
        )
...
def create_arg_parser():
    ...
    parser.add_argument("--eval", action="store_true", help="Use public.jsonl and score against public answers.")
    ...
    parser.add_argument("--max_num_seqs", type=int, default=64, help="Maximum number of vLLM sequences.")
    parser.add_argument(
        "--adapter",
        default="none",
        help=(
            "LoRA adapter to use: none, public, private, public_private, "
            "a Hugging Face repo id, or a local adapter path."
        ),
    )
    parser.add_argument("--data_path", default=None, help="Override dataset path.")
    parser.add_argument("--use_wandb", action="store_true", help="Log results to W&B like run_task.py.")
    return parser
